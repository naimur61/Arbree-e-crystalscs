...
export const Password = ({
  ...
  const renderStrengthRules = () => {
    ...
    if (passwordRules.every((rule: PasswordRule) => rule.test(passwordValue)))
      return null;
      ...
      <ul className="mt-2 space-y-1 text-sm">
        {passwordRules.map((rule: PasswordRule, index: number) => {
          ...
          return (
            ...
            >
              <span
                className={`w-2 h-2 rounded-full inline-block ${passed ? "bg-green-500" : "bg-muted-foreground/30"}`}
              />
              {rule.label}
            </li>
          );
          ...
          return (
            <FormItem>
              ...
              <FormControl>
                {renderInput({
                  ...field,
                  value: passwordValue || field.value || "",
                })}
              </FormControl>
              {renderStrengthRules()}
              <FormMessage>

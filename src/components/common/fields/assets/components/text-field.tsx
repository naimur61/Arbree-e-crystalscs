...
export const Text = ({
                    ...
                    <div className="w-full relative">
                      ...
                      <Input
                        ...
                        onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                          if (isArray) {
                            const arr = event.target.value.split(/[\s,]+/).map((v: string) => v.trim()).filter((v: string) => v.length > 0);
  ...
  return (
    <>
      ...
      <div className="w-full relative">
        {leftIcon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">{leftIcon}</div>}
        <Input className={`${leftIcon ? 'pl-10' : 'pl-4'} ${rightIcon ? 'pr-10' : 'pr-4'}`} value={value} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setValue?.(e.target.value || '')} type="text" placeholder={placeholderText} />
        {rightIcon && <div className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground cursor-pointer">{rightIcon}</div>}
      </div>
    </>
  );
